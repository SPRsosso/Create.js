...
if (canvas) {
    ...
    const polygon = new CreateJS.ConvexPolygon(500, 500, ...polygonPoints, new CreateJS.Vec2()).fillColor("red").fill();
    console.log(polygon);
    ...
    CreateJS.TimeHandler.tick(fps, ( currentTick, dt ) => {
        ...
        rect.translate(velocity.x, velocity.y);
        velocity.zero();
            
        game.run([ line, rect, rect2, rect3, polygon ]);
    });
}
