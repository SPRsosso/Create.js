...
async function start() {
    ...
    const loadedImages: { [key: string]: CreateJS.Image} = {
    ...
    if (canvas) {
        ...
        const bodies: CreateJS.Physics.Rigidbody[] = [];
        for (let i = 0; i < 5; i++) {
        ...
        const mouseHandler = new CreateJS.MouseEvent.Handler<CreateJS.Sprite>();
        ...
        CreateJS.TimeHandler.tick(fps, ( currentTick, dt ) => {
            ...
            if (velocity < 0) newDir = "left";
            if (velocity > 0) newDir = "right";
            if (velocity !== 0) player.setImage(loadedImages[`walking_${newDir}`]);
            if (velocity === 0) player.setImage(loadedImages[`idle_${newDir}`]);
            direction = newDir;
